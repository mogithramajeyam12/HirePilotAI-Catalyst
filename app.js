...
Base the score on: responsiveness, enthusiasm, questions asked, salary comfort, role fit expressed.`;
  ...
  try {
    ...
    const recruiterTurns = chatHistory.filter(m => `${m.role}`.toLowerCase().includes('recruiter')).length;
    const candidateTurns = chatHistory.filter(m => `${m.role}`.toLowerCase().includes('candidate')).length;
    const text = chatHistory.map(m => `${m.content || ''}`.toLowerCase()).join(' ');
    const positiveHits = ['interested', 'sounds good', 'happy', 'available', 'let us', 'call', 'interview', 'move forward']
      .filter(k => text.includes(k)).length;
    const questionHits = (text.match(/\?/g) || []).length;

    let interestScore = estimateInterestFromProfile(candidate).interest_score;
    interestScore += Math.min(recruiterTurns + candidateTurns, 8);
    interestScore += positiveHits * 3;
    interestScore += Math.min(questionHits, 4) * 2;
    interestScore = Math.max(35, Math.min(95, interestScore));

    const level = interestScore >= 75 ? 'high' : interestScore >= 55 ? 'medium' : 'low';
    return {
      interest_score: interestScore,
      interest_level: level,
      key_signals: ['Conversation engagement observed', `${candidateTurns} candidate replies received`],
      concerns: level === 'low' ? ['Limited positive buying signals in chat yet'] : [],
      recommended_action: level === 'high' ? 'Fast-track interview scheduling' : 'Continue probing motivation and timing'
    };
  }
}

function estimateInterestFromProfile(candidate) {
  let score = 45;
  if (candidate?.open_to_work) score += 18;
  if (candidate?.github_activity === 'high') score += 8;
  if (candidate?.github_activity === 'medium') score += 4;

  const days = parseInt((candidate?.last_active || '').match(/\d+/)?.[0] || '7', 10);
  if (days <= 2) score += 8;
  else if (days <= 5) score += 5;
  else if (days >= 10) score -= 4;

  return {
    interest_score: Math.max(35, Math.min(85, score))
  };
}
...
function ChatPanel({ candidate, jdTitle, onInterestScored }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      if (finalMessages.filter(m => m.sender === 'candidate').length >= 2) {
...
function CandidateCard({ candidate, rank, jdTitle, interestData, onInterestScored, parsedJD, scoreView = 'combined' }) {
  ...
  const fallbackInterest = estimateInterestFromProfile(candidate).interest_score;
  const effectiveInterest = interest !== null ? interest : (scoreView === 'combined' ? fallbackInterest : null);
  const combined = effectiveInterest !== null ? Math.round((candidate.match_score * 0.55 + effectiveInterest * 0.45)) : '–';
      ...
      <div className="cand-top">
        ...
        <div className="scores-col" onClick={e => e.stopPropagation()}>
          ...
          {scoreView === 'combined' && (
            <>
              <ScorePill label="Match" value={candidate.match_score} type="match" />
              <ScorePill label="Interest" value={effectiveInterest} type="interest" />
              <ScorePill label="Combined" value={combined} type="combined" />
            </>
          )}
        </div>
      </div>
...
function App() {
  ...
  const handleScout = async () => {
    ...
    try {
      ...
      const normalized = (Array.isArray(cands) ? cands : []).map((c, i) => ({ ...c, id: c?.id || `c${i + 1}` }));
  ...
  const sortedCandidates = [...candidates].sort((a, b) => {
    ...
    const ia = interestMap[a.id]?.interest_score ?? estimateInterestFromProfile(a).interest_score;
    const ib = interestMap[b.id]?.interest_score ?? estimateInterestFromProfile(b).interest_score;
    const combA = Math.round(a.match_score * 0.55 + ia * 0.45);
    const combB = Math.round(b.match_score * 0.55 + ib * 0.45);
    return combB - combA;
  });

  const filteredCandidates = sortedCandidates.filter(c => {
    if (tab === 'strong') return c.match_score >= 75;
    if (tab === 'engaged') return !!interestMap[c.id];
    if (tab === 'open') return c.open_to_work;
    return true;
  });

  const shortlistCandidates = sortedCandidates.filter(c => {
    const int = interestMap[c.id]?.interest_score ?? estimateInterestFromProfile(c).interest_score;
    const comb = Math.round(c.match_score * 0.55 + int * 0.45);
    return comb >= 70;
  }).slice(0, 6);
          ...
          <div className="page">
            ...
            <div className="stats-row">
              ...
              <div className="stat-card">
                ...
                <div className="stat-val">{Object.keys(interestMap).length}</div>
                <div className="stat-sub stat-up">↑ {Object.values(interestMap).filter(d=>d.interest_level==='high').length} high interest</div>
              </div>
              <div className="stat-card">
              ...
              <>
                ...
                <div className="candidate-list">
                  {sortedCandidates.slice(0,3).map((c,i) => (
                    <CandidateCard key={c.id} candidate={c} rank={i+1} jdTitle={parsedJD?.title || 'Position'} interestData={interestMap[c.id]} onInterestScored={handleInterestScored} parsedJD={parsedJD} scoreView="combined" />
                  ))}
                </div>
              </>
            )}
              ...
              <>
                ...
                <div className="candidate-list">
                  {filteredCandidates.map((c, i) => (
                    <CandidateCard key={c.id} candidate={c} rank={sortedCandidates.indexOf(c)+1} jdTitle={parsedJD?.title||'Position'} interestData={interestMap[c.id]} onInterestScored={handleInterestScored} parsedJD={parsedJD} scoreView={sortBy} />
                  ))}
                </div>
              </>
            )}
                  ...
                  <table className="shortlist-table">
                    ...
                    <tbody>
                      {shortlistCandidates.map((c, i) => {
                        const chatInt = interestMap[c.id]?.interest_score ?? null;
                        const estimatedInt = estimateInterestFromProfile(c).interest_score;
                        const int = chatInt !== null ? chatInt : estimatedInt;
                        const comb = Math.round(c.match_score * 0.55 + int * 0.45);
                        const intLevel = interestMap[c.id]?.interest_level || (int >= 75 ? 'high' : int >= 55 ? 'medium' : 'low');
                        return (
                          <tr key={c.id}>
                            ...
                            <td>
                              {chatInt !== null ? (
                                ...
                                <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                                  <span style={{fontWeight:600,color:'var(--accent3)'}}>{int}</span>
                                  <span style={{fontSize:'11px',padding:'2px 8px',borderRadius:'20px',background:'rgba(124,111,239,0.15)',color:'var(--accent2)'}}>estimated</span>
                                </div>
                              )}
                            </td>
          ...
          <div className="page">
            ...
            <div className="jd-card" style={{marginBottom:'20px'}}>
              ...
              <div style={{background:'var(--bg3)',borderRadius:'10px',padding:'24px',marginTop:'12px',fontFamily:'monospace',fontSize:'13px',lineHeight:'2',color:'var(--text2)'}}>
                ...
                <div style={{paddingLeft:'32px',color:'var(--text3)'}}>→ Produces interest signals after early chat turns</div>
